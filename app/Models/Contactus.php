...
class Contactus extends Model
{
    ...
    public function storeFrontContactUs($r){
      ...
      $obj->company_name = $input['company_name'];
      $obj->save();

      if ($obj !=null) {

        $data  = \App\Models\EmailTemplate::where('title','send_mail_to_user_for_request')->where('status',1)->first();
        ...
        $adminEmail  = \App\Models\EmailTemplate::where('title','send_mail_to_admin_for_request')->where('status',1)->first();
        ...
        if ($adminEmail !=null) {
          ...
          $email_body = str_replace(array('###NAME###','###EMAIL###','###COMPANY_NAME###','###PHONE###','###MESSAGE###','###SITE_URL###','###SITE_NAME###'), array($obj->name,$obj->email,$obj->company_name,$obj->phone,$obj->message,route('front.home'),trans('lang_data.app_name')), $template);
        ...
        $msg = trans('lang_data.thank_you_for_request_catalogue_label');
