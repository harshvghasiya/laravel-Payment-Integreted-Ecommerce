...
@else
<title>{{trans('lang_data.app_name')}}</title>
@endif
<style type="text/css">
        body {
            margin: 0;
...
<img align="center" alt="{{trans('lang_data.app_name')}}" border="0" class="center fixedwidth" 
src="{{$setting->getSettingLogoImageUrl()}}" style="text-decoration: none; -ms-interpolation-mode: bicubic; height: auto; border: 0; width: 100%; max-width: 150px; display: block;" title="{{trans('lang_data.app_name')}}" width="352"/>
</div>
<?php
...
@if(isset($socialMediaContent) && !empty($socialMediaContent))
...
@foreach($socialMediaContent as $key=>$v)
    <td style="word-break: break-word; vertical-align: top; padding-bottom: 0; padding-right: 5px; padding-left: 5px;" valign="top">
        <a href="{{$v->url}}" target="_blank"><img alt="{{$v->title}}" height="32" src="{{$v->getSocialMediaImageUrl()}}" style="text-decoration: none; -ms-interpolation-mode: bicubic; height: auto; border: 0; display: block;" title="{{$v->title}}" width="32"/>
        </a>
    </td>
@endforeach
</tr>
...
<p style="font-size: 12px; line-height: 1.2; word-break: break-word; text-align: left; font-family: inherit; mso-line-height-alt: 14px; margin: 0;"><span style="color: #95979c; font-size: 12px;">{{trans('lang_data.app_name')}} Copyright © {{date('Y')}}</span></p>
</div>
</div>
</div>
</div>
